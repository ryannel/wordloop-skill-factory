# Bet Design Studio Documentation

The Bet Design Studio is the canonical workspace for managing a bet through its full lifecycle: Specify → Discover → Design → Implement → Validate. Each phase has a structured document with clear conventions. This reference governs how to author and maintain those documents.
...
The discovery document answers: **What already exists in the codebase that is relevant to this bet?**
...
For bets with real-time data flows, include a streaming cadences table that summarises update frequency and delivery path for each data type:
...
The validation document answers: **How do we know the bet is correct?**
...
The Rollback Plan describes the exact steps to undo the bet if production issues arise — not a generic statement, but a concrete sequence for each affected layer (database migration rollback, bet flag kill switch, API versioning).

---

## Lifecycle Progression

A bet moves through phases in order. No phase begins until the previous document is complete and agreed:
